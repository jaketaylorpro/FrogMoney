    ...
    async.parallel([
        function(callback) {
            req.app.get('db').users.findOne({id:req.cookies.id},function(err,obj){
                logger.trace('user results: '+moment().toISOString()+': '+util.inspect(err)+','+util.inspect(obj));
                callback(err,obj);
            });
        },
        function(callback) {
            req.app.get('db').expenses.getExpensesForUserId(req.cookies.id,function(err,obj){
                logger.trace('expenses results: '+moment().toISOString()+': '+util.inspect(err)+','+util.inspect(obj));
                callback(err,obj);
            });
        }
    ],function(err,results){

        logger.trace('home callback: '+moment().toISOString()+': '+util.inspect(err)+','+util.inspect(results));
        res.render('home', {
